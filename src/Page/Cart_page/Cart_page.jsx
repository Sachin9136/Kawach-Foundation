      ...
      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-3 gap-4 mb-20 px-4 md:px-20">
              ...
              {cart.map((item) => (
                <tr key={item.id} className="border-b">
                  <td className="p-4 flex items-center">
                    <img src={item.image} alt={item.name} className=" rounded-xl mr-4" width="80px"/>
                    {item.name}
                  </td>
                  <td className="p-4 text-xl text-[#5D5C5C] font-semibold">₹{item.price.toFixed(2)}</td>
                  <td className="p-4">
                  ...
                  <td className="p-4 text-xl text-[#5D5C5C] font-semibold">₹{(item.price * item.quantity).toFixed(2)}</td>
        ...
        <tbody>
          <tr className="">
            ...
            <td className="px-4 pt-5 text-right text-[#5D5C5C] font-semibold">₹{calculateSubtotal()}</td>
          </tr>
          <tr className="border-b">
            <td className="px-4 py-3 text-left text-xl font-semibold">Shipping Fee</td>
            <td className="px-4 py-3 text-right text-[#5D5C5C] font-semibold">₹{shippingFee.toFixed(2)}</td>
          </tr>
          <tr className="border-b">
            <td className="p-4 text-left text-xl font-semibold">Total</td>
            <td className="p-4 text-right text-[#5D5C5C] font-bold text-lg">₹{calculateTotal()}</td>
          </tr>
          <tr className="">
            <td className="p-2 py-4">
                <Link to={"/checkout"}>
                    <button className=" bg-[#FFED05] hover:bg-yellow-500 text-2xl font-bold py-3 px-6 rounded-lg">
                        Proceed to Checkout
                    </button>
                </Link>
            </td>
